...
export class HistoricoComponent extends MailboxClass implements OnInit, OnDestroy {
  ...
  setMailbox(): void {
    if (isNullOrUndefined(this.organizationalUnitId))
      return;

    this.mailboxService.historico(this.organizationalUnitId).subscribe(
        response => {
          if(response['status']) {
